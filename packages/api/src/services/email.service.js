...
class EmailService {
    ...
    constructor() {
        ...
        if (!this.apiKey || !this.apiSecret) {
            console.warn('⚠️  Email service not configured. Alerts will not be sent via email.');
    ...
    getMailjetClient() {
        if (!this.apiKey || !this.apiSecret) {
            throw new Error('Email service not configured. Set MJ_APIKEY_PUBLIC and MJ_APIKEY_PRIVATE environment variables.');
        }
        if (!this.mailjet) {
     ...
     */
    async sendInvitationEmail(email, token, organizationName, inviterName, inviteeFirstName) {
...
<body>
  <div class="container">
     ...
     */
    async sendEmailWithRecipients(options) {
        const maxRetries = 3;
    ...
    async sendEmail(options) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (isLastAttempt || !isRetryable) {
                    throw new Error(`Failed to send email: Unsuccessful: Error Code: "${error.code}" Message: "${error.message}"`);
                }
