...
class SchedulerService {
    ...
    constructor(db) {
        ...
        this.exclusionRepo = new shared_1.ScheduleExclusionRepository(db);
    }
    /**
     ...
     */
    async calculateNextRunTime(schedule, from = new Date()) {
        if (!schedule.enabled) {
            return null;
        }
        // Convert 'from' to schedule's timezone
        const fromDateTime = luxon_1.DateTime.fromJSDate(from, {
        ...
        if (schedule.valid_from && nextRun < luxon_1.DateTime.fromJSDate(schedule.valid_from)) {
            nextRun = luxon_1.DateTime.fromJSDate(schedule.valid_from, {
                zone: schedule.timezone || 'UTC',
            });
        }
        if (schedule.valid_until && nextRun > luxon_1.DateTime.fromJSDate(schedule.valid_until)) {
     ...
     */
    calculateMinutely(schedule, from) {
     ...
     */
    calculateMonthly(schedule, from) {
        const executionTimes = schedule.execution_times || ['00:00'];
        const [hour, minute] = executionTimes[0].split(':').map(Number);
        if (schedule.day_of_month) {
            // Specific day of month (e.g., "1st", "15th", "last")
            return this.nextOccurrenceOfDayOfMonth(from, schedule.day_of_month, hour, minute);
        }
        else if (schedule.week_of_month && schedule.days_of_week) {
            // Specific week and day (e.g., "2nd Tuesday", "Last Friday")
            return this.nextOccurrenceOfWeekOfMonth(from, schedule.week_of_month[0], schedule.days_of_week[0], hour, minute);
        }
        else {
     ...
     */
    nextOccurrenceOfDayOfMonth(from, dayOfMonth, hour, minute) {
     ...
     */
    nextOccurrenceOfWeekOfMonth(from, weekOfMonth, dayOfWeek, hour, minute) {
        ...
        return this.nextOccurrenceOfWeekOfMonth(from.plus({ months: 1 }).startOf('month'), weekOfMonth, dayOfWeek, hour, minute);
    }
    /**
     ...
     */
    async skipExclusions(schedule, nextRun) {
        ...
        while (attempts < maxAttempts) {
            ...
            current = await this.calculateNextRunTime(schedule, current.plus({ days: 1 }).toJSDate()).then((date) => (date ? luxon_1.DateTime.fromJSDate(date, { zone: schedule.timezone }) : null));
            if (!current) {
                return null;
            }
            attempts++;
        }
        console.warn(`Could not find non-excluded date for schedule ${schedule.id} after ${maxAttempts} attempts`);
        return null;
    }
    /**
     * Check if a date is excluded
     */
    isDateExcluded(date, exclusions) {
        for (const exclusion of exclusions) {
            if (exclusion.excluded_date) {
                // Single date exclusion
                const excludedDate = luxon_1.DateTime.fromJSDate(exclusion.excluded_date, {
                    zone: date.zone,
                });
                ...
                const from = luxon_1.DateTime.fromJSDate(exclusion.excluded_from, {
                    zone: date.zone,
                });
                const to = luxon_1.DateTime.fromJSDate(exclusion.excluded_to, {
                    zone: date.zone,
                });
     ...
     */
    async shouldRunNow(schedule, lastRunTime) {
        if (!schedule.enabled) {
            return false;
        }
        const now = new Date();
