...
class SLAMonitorService {
    ...
    constructor(db) {
        ...
        this.fileTrackingRepo = new shared_1.FileTrackingRepository(db);
        this.scheduleRepo = new shared_1.ScheduleRepository(db);
        this.schedulerService = new scheduler_service_1.SchedulerService(db);
    }
    /**
     ...
     */
    async createExpectedFileRecords(lookAheadHours = 24) {
        const now = new Date();
        const lookAheadUntil = luxon_1.DateTime.fromJSDate(now).plus({ hours: lookAheadHours }).toJSDate();
        ...
        for (const watcher of watchers) {
            try {
                ...
                while (recordsCreatedForWatcher < MAX_RECORDS_PER_WATCHER) {
                    ...
                    const nextRun = await this.schedulerService.calculateNextRunTime(schedule, currentTime);
                    ...
                    const slaDeadline = watcher.sla_enabled && slaThresholdMinutes
                        ? luxon_1.DateTime.fromJSDate(normalizedNextRun)
                    ...
                    const oneMinuteBefore = luxon_1.DateTime.fromJSDate(normalizedNextRun).minus({ minutes: 1 }).toJSDate();
                    const oneMinuteAfter = luxon_1.DateTime.fromJSDate(normalizedNextRun).plus({ minutes: 1 }).toJSDate();
                ...
                if (recordsCreatedForWatcher >= MAX_RECORDS_PER_WATCHER) {
                    console.warn(`⚠️  Safety limit reached: Created ${MAX_RECORDS_PER_WATCHER} records for watcher ${watcher.id} (${watcher.name}). ` +
                        `This may indicate a very high-frequency schedule.`);
                }
                // Log records created for this watcher
                if (recordsCreatedForWatcher > 0) {
                    console.log(`   ✓ Watcher "${watcher.name}" (ID: ${watcher.id}): Created ${recordsCreatedForWatcher} expected records`);
                }
            }
            catch (error) {
                console.error(`Error creating expected file record for watcher ${watcher.id}:`, error);
            }
        }
     ...
     */
    async checkForMissingFiles(lookBackHours = 24) {
        const now = new Date();
        const lookBackFrom = luxon_1.DateTime.fromJSDate(now)
        ...
        for (const record of overdueRecords) {
            try {
                ...
                console.error(`Error checking file tracking record ${record.id}:`, error);
            }
        }
        ...
        for (const record of nonSLAOverdueRecords) {
            try {
                ...
                const missingSinceThreshold = luxon_1.DateTime.fromJSDate(expectedAt).plus({ hours: 1 }).toJSDate();
                ...
                console.error(`Error marking non-SLA file tracking record ${record.id} as missing:`, error);
            }
        }
     ...
     */
    async matchDetectedFile(watcherId, fileName, filePath, fileSize, fileUploadedAt) {
        ...
        const windowStart = luxon_1.DateTime.fromJSDate(fileUploadedAt).minus({ hours: 2 }).toJSDate();
        const windowEnd = luxon_1.DateTime.fromJSDate(fileUploadedAt).plus({ hours: 2 }).toJSDate();
        ...
        if (pendingRecords.length === 0) {
            console.log(`   ℹ️  No pending expectation found for file ${fileName} (uploaded at ${fileUploadedAt.toISOString()})`);
        ...
        let minTimeDiff = Math.abs(new Date(closestRecord.expected_at).getTime() - fileUploadedAt.getTime());
        for (const record of pendingRecords) {
            const timeDiff = Math.abs(new Date(record.expected_at).getTime() - fileUploadedAt.getTime());
        ...
        const status = isLate ? 'late' : 'arrived';
        ...
        if (isLate) {
            ...
            console.log(`   ⚠️  Late file: ${fileName} (expected by ${closestRecord.sla_deadline}, uploaded at ${fileUploadedAt.toISOString()})`);
        }
        else {
            const arrivedEarly = fileUploadedAt < new Date(closestRecord.expected_at);
            if (arrivedEarly) {
                console.log(`   ✅ File arrived early: ${fileName} (expected at ${closestRecord.expected_at}, uploaded at ${fileUploadedAt.toISOString()})`);
            }
            else {
                console.log(`   ✅ File arrived on time: ${fileName} (expected at ${closestRecord.expected_at}, uploaded at ${fileUploadedAt.toISOString()})`);
            }
        }
    }
    /**
     ...
     */
    async getSLASummary(periodStart, periodEnd) {
        const summary = await this.fileTrackingRepo.getSLASummary(periodStart, periodEnd);
        return summary;
    }
    /**
     * Get missing file alerts for a specific watcher
     */
    async getMissingFileAlerts(watcherId, periodStart, periodEnd) {
        return this.fileTrackingRepo.getMissingFileAlerts(watcherId, periodStart, periodEnd);
    }
    /**
    ...
    async cleanupOldRecords(retentionDays = 90) {
        const cutoffDate = luxon_1.DateTime.now().minus({ days: retentionDays }).toJSDate();
        const deletedCount = await this.db('file_tracking')
            .where('expected_at', '<', cutoffDate)
            .del();
        console.log(`🗑️  Cleaned up ${deletedCount} file tracking records older than ${retentionDays} days`);
        return deletedCount;
    }
}
